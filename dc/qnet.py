...
class CQLCritic(nn.Module):
    def __init__(self, state_dim, action_dim, hidden_dim=256, gamma=0.95, min_q_weight=1.0, temp=1.0, n_random=10, max_q_backup=False):
    ...
    def forward_target(self, state, action):
        x = torch.cat([state, action], dim=-1)
    ...
    def loss(self, trajectories, goal, ema_model):
        ...
        pred_q1, pred_q2 = self.forward(s, a)
        if self.max_q_backup:
            # data
            ns_rpt = torch.repeat_interleave(ns, repeats=10, dim=0)
            na_rpt, *_ = self._normalized_transition(ema_model(ns_rpt, goal))
            pred_targ_q1, pred_targ_q2 = self.forward_target(ns_rpt, na_rpt)
            targ_q1 = targ_q1.view(batch_size, 10).max(dim=1, keepdim=True)[0]
            targ_q2 = targ_q2.view(batch_size, 10).max(dim=1, keepdim=True)[0]
            ...
            pred_targ_q1_new, pred_targ_q2_new = self.forward_target(ns_rpt_new, na_rpt_new)
        ...
        rand_q1, rand_q2 = self.forward(s_new_temp, random_actions)
        
        # next
        next_q1, next_q2 = self.forward(s, na_new)
        ...
        return q1_loss_cql, q2_loss_cql, q1_loss, q2_loss, q_normed
